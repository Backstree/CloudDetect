...
bool CBilateralFiltering::BilateralFiltering(GDALDataset * pSrcDatasetR, GDALDataset * pDrcDatasetR,int r)
{
	...
	Float32 *pDRBuf = new Float32[BX*BY]; 
	//存放整幅影像的数组，读取使用
	Float32 *pDstBuf=new Float32 [BX*BY];
	//读取整幅影像->pDstBuf
	//if (pSrcBandR->RasterIO(GF_Read,0,0,nWidth,nHeight,pDstBuf,nWidth,nHeight,GDT_Float64,0,0) != CE_None)
	//{
	//	delete [] pDstBuf;
	//	pDstBuf = 0;
	...
	int jj=0;
	//把整幅影像转换为二维数组
	//OneArr2TwoArr(pDstBuf,pDDsdBuf,0,nHeight,0,nWidth);
	/*delete [] pDstBuf;
	pDstBuf = 0;*/
	jj=0;
	//读取2*r+1的图像数组
	...
	for (int iYBL=0;iYBL<YBL;iYBL++)
	{
		for (int iXBL=0;iXBL<XBL;iXBL++)
		{
			...
			if (pSrcBandR->RasterIO(GF_Read,iYBL,iXBL,BY,BX,pDstBuf,BY,BX,GDT_Float64,0,0) != CE_None)
			{
				delete [] pDstBuf;
				pDstBuf = 0;
				std::cout<<"读取文件出错！"<<std::endl;
			...
			for (iYBlock=0; iYBlock<BY; iYBlock++)
			{
				for (iXBlock=0; iXBlock<BX; iXBlock++)
				{	
					if (pDDsdBuf[iYBlock][iXBlock]!=0)
					{
						...
						//读取一个2*r+1的影像块
						//边界判断
						if (iXBlock+r<BX&&iYBlock+r<BY&&iXBlock-r>=0&&iYBlock-r>=0)
						{
			...
			if (pDstBandR->RasterIO(GF_Write,iYBL,iXBL,BY,BX,pDRBuf,BY,BX,GDT_Float64,0,0) != CE_None)
			{
	...
	TwoArr2OneArr(pDDDsdBuf,pDRBuf,0,nHeight,0,nWidth);
	for(int i = 0; i < nHeight; i++)
	{
		delete [] pDDDsdBuf[i];
	}
